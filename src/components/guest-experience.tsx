...
export default function GuestExperience({ initial }: { initial: GuestData }) {
  ...
  }, [openedKey, data.token]);
  const openInvitation = () => {
    setOpening(true);
    setOpened(true);
    try {
      if (data.token !== "design-preview")
  ...
  return (
    <WeddingMediaProvider wedding={data.wedding} token={data.token}>
      <div
        ref={root}
      ...
      >
        {data.preview && (
          <div className="guest-preview-banner" role="status">
            {data.token === "design-preview"
              ? locale === "en"
                ? "Design preview · sample guest details"
                : "Vista del diseño · invitados de ejemplo"
              : locale === "en"
                ? `Read-only preview · ${data.household} · expires in one hour`
                : `Vista previa de solo lectura · ${data.household} · caduca en una hora`}
            <a
              href={`/studio/${data.token === "design-preview" ? "experience" : "setup"}?wid=${data.wedding.id}`}
              target={data.token === "design-preview" ? "_top" : undefined}
            >
          ...
          <>
            ...
            <main id="main">
              ...
              >
                <GuestCountdown
                  target={
                    chronologicalEvents[0]?.starts_at || data.wedding.date
                  }
                  locale={locale}
